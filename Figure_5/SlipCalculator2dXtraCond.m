function [Ds,Dn,Pxx,Pyy,Pxy,StrainEnergy,A]=SlipCalculator2dXtraCond...
    (MidPoint,HalfLength,Pxx,Pyy,Pxy,TnIn,TsIn,nu,E,halfspace,LineNormalVector,strain,Fdisp,Mu,Sf,Option,...
    P1,P2,StrainEnergyDir,ThetaIn,TipIndxs,noNewels,FuncArray)
...
%           chamber.
%           'F'=Inhomogeneous.
%           'H'=Hanwen's no vertical displacement along bottom boundary
%
% Arguments: (output)
...
if Option=='B' || Option=='C' || Option=='D' || Option=='F' || Option=='H'
...
if Option~='F'    
    ...
    if Option=='H'
            [StressInf,Tn,Ts,NUM]=FixingDisp_InfRowSwap2dHanwen(StressInf,DispInf,NUM,Tn,Ts,Fdisp);
            clear DispInf
    else
        if any(Fdisp)==1
...
if Option=='B' || Option=='E' || Option=='C' || Option=='H'   
  
    Ats = [-StressInf.DnTn,-StressInf.DsTn];            
    %StressInf = rmfield(StressInf,{'DnTn','DsTn'});   
...
if StrainEnergyDir==1
    if numel(ThetaIn)>0
        AddTractionToNewEls=0; disp('Provide as function to compute Ts and Tn: line 304 slip calc!')
        %Will supply number of strain energys based on number of 'thetas' you
        %provide
        [StrainEnergy]=StrainEnergySlipCalcHomoMat(D,StressInf,HalfLength,MidPoint,P1,P2,...
            halfspace,nu,E,Fdisp,TipIndxs,noNewels,ThetaIn,Tn,Ts,TnIn,TsIn,AddTractionToNewEls,FuncArray);
    else
        StrainEnergy=NaN;
    end
    % %% If you are going to pick the best new prop direction (1 tip):
    % scatter(rad2deg(ThetaIn),StrainEnergy);
    % [a,Indx]=min(StrainEnergy);
    % answer=rad2deg(ThetaIn(Indx));
    % MxAng=max(ThetaIn);
    % ThtHgh=linspace(-MxAng,MxAng,500);
    % vq2 = interp1(rad2deg(ThetaIn),StrainEnergy,ThtHgh,'spline');
    % [a,Indx]=min(vq2);
    % ThetaNew=(ThtHgh(Indx));
else
    StrainEnergy=NaN;
end
