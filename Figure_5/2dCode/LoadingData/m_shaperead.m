function [pointsxy,mystruct]=m_shaperead(fname,UBR);
...
%   what is there. The data its elf is usually
...
pp=(M(1).ncst); %extracting from array
...
%     Els=padcat(Els,NaNIndex(i)+1:NaNIndex(i+1)-1);
% end
for i =(1:numel(NaNIndex)-1)
    stringname{i} = strcat('line', num2str(i));
    mystruct.(stringname{i}) = NaNIndex(i)+1:NaNIndex(i+1)-1;
end
