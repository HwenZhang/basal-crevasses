function [Sxx,Syy,Sxy] = K2StressDistro(K2, X,Y, TipDip)
...
%
% Example usage:
% %Reproduce P&F Fig9.31 box b)
% K2=1
% theta=linspace(-pi,pi,100);
% r=1;
% [X,Y] = pol2cart(theta,r);
% [Sxx,Syy,Sxy] = K2StressDistro(K2, X,Y, 0);
% figure;hold on
% plot(theta,Sxx)
% plot(theta,Syy)
% plot(theta,Sxy)
...
Sxx=Cons1.*(-sin(theta/2).*(2+cos(theta/2).*cos((3*theta)/2)));
Syy=Cons1.*(sin(theta/2).*(cos(theta/2).*cos((3*theta)/2)));
Sxy=Cons1.*(cos(theta/2).*(1-sin(theta/2).*sin((3*theta)/2)));
