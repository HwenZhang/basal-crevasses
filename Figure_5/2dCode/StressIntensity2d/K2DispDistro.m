function [Ux,Uy] = K2DispDistro(K2, X,Y, TipDip,mu,nu)
%K2StressDistro Distribution of displacements around the tip of a crack due to
%stress intensity K2
%
% usage #1:
% [Ux,Uy] = K2DispDistro(K2, X,Y, TipDip,mu,nu)
%
% Arguments: (input)
...
Cons1=K2./mu;
Cons2=sqrt(r./(2*pi));
Ux=Cons1.*Cons2.*(sin(theta./2).*(2-(2*nu)+cos(theta./2).^2));
Uy=Cons1.*Cons2.*(cos(theta./2).*(-1+(2*nu)+sin(theta./2).^2));
