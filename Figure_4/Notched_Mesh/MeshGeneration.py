...
# pizometric head dominate by flotation fraction
H_w=0.65
# varying cracklength
vlambda1 = np.arange(0.05,0.1,0.01)
vlambda2 = np.arange(0.1,1,0.02)
vlambda = np.append(vlambda1,vlambda2)
Nlambda = np.shape(vlambda)[0]
# varying w
vw = np.arange(0.1,1.1,0.1)
Nw = np.shape(vw)[0]

# folder index
for j,w in enumerate(vw):
    for i,C in enumerate(vlambda):
        ...
        Lc2 = min([0.05,Lc1*40])
        # Lc2 = 0.05
        # 0.05 case is stable but less accurate when cracklength is small
        ...
        gmsh.model.mesh.field.setNumber(4, "DistMin", w / 10.0)
        gmsh.model.mesh.field.setNumber(4, "DistMax", w / 4.0)
        ...
        meshio.write("Notched_mesh_C_"+str(i)+"_w_"+str(j)+".xdmf", triangle_mesh)
        ...
        line_mesh = meshio.Mesh(points=mesh_from_file.points,
                              ...
                              field_data=mesh_from_file.field_data)
        line_mesh.prune_z_0()
        meshio.write("Notched_facet_mesh_C_"+str(i)+"_w_"+str(j)+".xdmf", line_mesh)